...
- `day_of_week` — select-enum `mon…sun` вместо числа 0–6 (в PocketBase у необязательных
  number-полей нет различия «0 / не задано»).
- По той же причине `target_reps_min/max`, `target_weight`, `rest_seconds`, `reps` и т.п. —
  необязательные number-поля, где `0` означает «не задано».
...
| `day_of_week` | select   | `mon`…`sun`, опционально              |
